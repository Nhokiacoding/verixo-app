...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!emailError && !passwordError && !adminCodeError) {
      setIsLoading(true);
      try {
        const response = await apiService.post('/auth/admin/login', formData, { auth: false });
        ...
        setErrors({
          email: error.message || 'Admin login failed. Please check your credentials.',
          password: '',
          adminCode: ''
        });
      } finally {
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-red-600 hover:bg-red-700 disabled:bg-red-400 text-white font-semibold py-3 px-4 rounded-lg transition-all duration-300 transform hover:scale-105 disabled:scale-100 shadow-lg hover:shadow-xl flex items-center justify-center"
          >
            {isLoading ? (
              <>
