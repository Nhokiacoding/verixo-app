...
const AdminDashboard = () => {
      ...
      try {
        ...
        try {
          const [dashboardResponse, usersResponse, activityResponse] = await Promise.all([
            ...
            apiService.get('/admin/users?limit=5&sortBy=createdAt&sortOrder=desc'),
            apiService.get('/admin/activity?limit=10')
          ]);

          const statsData = dashboardResponse.data || {};
          const usersData = usersResponse.data?.users || [];
          const activityData = activityResponse.data?.activities || [];

          setStats({
            totalUsers: statsData.users?.total || 0,
            totalRevenue: statsData.transactions?.revenue || 0,
            activeNumbers: statsData.numbers?.total || 0,
            successRate: parseFloat(statsData.numbers?.successRate || 0),
            availableNumbers: 100, // This would need to be calculated from BlissDigitals API
            soldNumbers: statsData.numbers?.successful || 0
          });
          ...
          setRecentTransactions(activityData
            ...
            .map(activity => ({
              id: activity.id,
              type: activity.action === 'fund_wallet' ? 'Wallet Funding' : 
                    activity.action === 'purchase_number' ? 'Number Purchase' : 
                    activity.action,
          ...
          // Set default stats for new installation
          setStats({
            totalUsers: 0,
            totalRevenue: 0,
            activeNumbers: 0,
  ...
  const recentTransactionsData = recentTransactions;
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-xl font-semibold text-gray-700 mb-2">Dashboard Error</h2>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={async () => {
                          try {
                            const response = await apiService.post('/admin/sync-inventory');
                            if (response.success) {
