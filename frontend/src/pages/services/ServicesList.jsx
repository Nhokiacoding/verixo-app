...
const ServicesList = () => {
  ...
  // Fetch services from API - no fallback to mock data
  useEffect(() => {
    const fetchServices = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        if (!token) {
          // No services available without authentication
          setServices([]);
        ...
        if (apiServices.length > 0) {
          ...
          const transformedServices = apiServices
            ...
            .map(service => ({
              ...
              pricing: {
                us: service.currentPrice || 3200,
                uk: service.currentPrice || 3200,
                ca: service.currentPrice || 3200,
                au: service.currentPrice || 3200,
                es: service.currentPrice || 3200,
                de: service.currentPrice || 3200
              }
            }));
