...
const USANumbers = () => {
  ...
  // USA-specific services with updated premium pricing
  const usaServices = [
    {
      id: 'wa',
      serviceCode: 'wa',
      ...
      popular: true,
      category: 'messaging',
      price: 3200 // Updated premium pricing
    },
    {
      id: 'tg',
      serviceCode: 'tg', 
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        // Try to get admin-set pricing from backend
        const token = localStorage.getItem('token');
        if (token) {
          try {
            ...
            if (response.ok) {
              ...
              const backendServices = data.data.services.map(service => ({
                ...
                stock: service.blissDigitals?.stock || 100
              }));
        ...
        if (savedServices) {
          try {
            ...
            console.error('Error loading admin pricing:', error);
          }
        }

        // Final fallback to default services
        if (services.length === 0) {
          setServices(usaServices);
        }
